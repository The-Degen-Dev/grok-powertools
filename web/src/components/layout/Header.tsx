...
function useBreadcrumb() {
  ...
  if (pathname === "/ops") return { label: "Ops", href: "/ops" };
...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-(--color-surface-200) bg-(--color-surface-0)/80 backdrop-blur-lg dark:border-(--color-surface-800) dark:bg-(--color-surface-950)/80">
      <div className="mx-auto flex h-14 max-w-screen-2xl items-center justify-between gap-[var(--space-2)] px-3 sm:px-4">
        {/* Left: Logo + Breadcrumb */}
        <div className="flex min-w-0 items-center gap-2">
          <Link
            href="/"
            className="flex min-w-0 items-center gap-2 font-[family-name:var(--font-display)] text-lg font-bold tracking-tight text-(--color-surface-900) dark:text-(--color-surface-100)"
          >
        ...
        {/* Right: Nav */}
        <nav className="flex min-w-0 shrink items-center gap-0 sm:gap-1">
          {NAV_ITEMS.map(({ href, icon: Icon, label }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className={`flex items-center gap-1.5 rounded-(--radius-btn) px-1.5 py-1.5 text-sm transition-colors duration-(--duration-fast) sm:px-2 xl:px-3 ${
                  isActive
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-(--radius-btn) px-1.5 py-1.5 text-sm text-(--color-surface-600) transition-colors duration-(--duration-fast) hover:bg-(--color-surface-100) dark:text-(--color-surface-400) dark:hover:bg-(--color-surface-800) sm:px-2 xl:px-3"
            title="Prompts"
          >
            <BookOpen className="h-4 w-4" />
            <span className="hidden xl:inline">Prompts</span>
          </button>
          <div className="hidden h-5 w-px bg-(--color-surface-200) dark:bg-(--color-surface-700) sm:mx-2 sm:block" />
          <ThemeToggle />
          <button
            type="button"
            onClick={() => setSettingsOpen(true)}
            className="rounded-(--radius-btn) p-1.5 text-(--color-surface-500) transition-colors duration-(--duration-fast) hover:bg-(--color-surface-100) hover:text-(--color-surface-700) dark:hover:bg-(--color-surface-800) dark:hover:text-(--color-surface-300) sm:p-2"
            title="Settings"
          >
