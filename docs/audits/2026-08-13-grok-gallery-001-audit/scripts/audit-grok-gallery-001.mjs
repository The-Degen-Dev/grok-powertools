...
const sourceAuditRoot = path.resolve(scriptsDir, '..');
...
async function ensureManifest() {
  const existing = await fs.stat(manifestPath).catch(() => null);
  if (existing?.isFile()) return;
  await ensureDir(auditRoot);
  await fs.writeFile(manifestPath, `${JSON.stringify({
    auditName: 'grok-gallery-001 correctness and metadata audit',
    auditPlanDate: nowIso().slice(0, 10),
    ...
    productionTarget: {
      ...
      d1Database: EXPECTED.d1Database,
...
async function scaffold() {
  ...
  await updateManifest((manifest) => {
    if (!manifest.executionStartedAt || manifest.executionStartedAt === '2026-08-13T00:00:00.000Z') {
      manifest.executionStartedAt = nowIso();
    }
  });
...
async function requireR2Env(mode = 'rawR2') {
  ...
  if (missing.length) {
    await addBlocker(mode, {
      ...
      missing,
      commandOrApiPath: 'S3-compatible ListObjectsV2/GetObject', // gitleaks:allow
...
function r2ListingFingerprint(listedObjects) {
  const hash = createHash('sha256');
  for (const object of [...listedObjects].sort((a, b) => a.key.localeCompare(b.key))) {
    hash.update(object.key);
    hash.update('\0');
    hash.update(String(Number(object.listedSize) || 0));
    hash.update('\0');
    hash.update(String(object.listedEtag || '').replaceAll('"', ''));
    hash.update('\0');
    hash.update(String(object.listedLastModified || ''));
    hash.update('\n');
  }
...
async function r2Inventory() {
  ...
  const listingGeneratedAt = nowIso();
  const listingFingerprint = r2ListingFingerprint(listedObjects);
  const listingPass = {
    generatedAt: listingGeneratedAt,
    bucket,
    productionPrefix: prefix,
    objectCount: listedObjects.length,
    totalListedBytes: listedObjects.reduce((sum, object) => sum + (Number(object.listedSize) || 0), 0),
    pageCount: pages.length,
    finalTruncatedState: pages.at(-1)?.isTruncated || false,
    listingFingerprint
  };
  await appendJsonl(path.join(auditRoot, 'logs', 'r2-listing-passes.jsonl'), listingPass);
  await recordEvidence(path.join(auditRoot, 'logs', 'r2-listing-passes.jsonl'));
  await writeJsonl(path.join(auditRoot, 'inventory', 'r2-objects-listing-checkpoint.jsonl'), listedObjects);
  await writeJson(path.join(auditRoot, 'inventory', 'r2-listing-checkpoint.json'), {
    ...listingPass,
    method: 'S3 ListObjectsV2 for the entire bucket before HeadObject enrichment',
    headConcurrency,
    hashConcurrency,
    hashBatchSize
  });
  ...
  const priorAuditRoot = configuredValue(
    'AUDIT_PRIOR_AUDIT_ROOT',
    '/Users/philipbankier/.codex/worktrees/production-r2-vault-audit/chrome-extension-powertools/docs/audits/2026-06-26-production-r2-vault-system-audit'
  );
  const priorObjects = await optionalJsonl(path.join(priorAuditRoot, 'inventory', 'r2-objects.jsonl'));
  const priorHashRows = await optionalJsonl(path.join(priorAuditRoot, 'inventory', 'r2-media-hashes.jsonl'));
  const priorObjectsByKey = new Map(priorObjects.map((object) => [object.key, object]));
  const priorHashesByKey = new Map(priorHashRows.map((row) => [row.objectKey, row]));
  const currentObjectsByKey = new Map(mediaObjects.map((object) => [object.key, object]));
  const hashRowsByKey = new Map();
  for (const row of existingHashRows) {
    const current = currentObjectsByKey.get(row?.objectKey);
    if (current && row.status === 'ok' && row.objectFingerprint === r2ObjectFingerprint(current)) {
      hashRowsByKey.set(row.objectKey, { ...row, evidenceSource: 'current-run-resume' });
    }
  }

  const priorReuseCandidates = [];
  for (const object of mediaObjects) {
    if (hashRowsByKey.has(object.key)) continue;
    const priorObject = priorObjectsByKey.get(object.key);
    const priorHash = priorHashesByKey.get(object.key);
    if (!priorObject || !priorHash || priorHash.status !== 'ok') continue;
    if (!sameR2ObjectVersion(object, priorObject) || Number(priorHash.bytesRead) !== Number(object.size)) continue;
    const reused = {
      ...priorHash,
      objectFingerprint: r2ObjectFingerprint(object),
      evidenceSource: 'verified-2026-06-28-reuse',
      reusedAt: nowIso()
    };
    hashRowsByKey.set(object.key, reused);
    priorReuseCandidates.push(object);
  }

  const revalidationObjects = selectHashRevalidationSample(priorReuseCandidates, 64);
  const revalidationRows = await mapLimit(revalidationObjects, hashConcurrency, async (object) =>
    hashR2MediaObject(client, GetObjectCommand, bucket, object, 'prior-hash-revalidation')
  );
  const hashMismatches = revalidationRows.filter((row) => {
    const prior = priorHashesByKey.get(row.objectKey);
    return row.status !== 'ok' || !prior?.sha256 || row.sha256 !== prior.sha256;
  });

  let fullRehashTriggered = false;
  if (hashMismatches.length > 0) {
    fullRehashTriggered = true;
    for (const object of priorReuseCandidates) hashRowsByKey.delete(object.key);
  } else {
    for (const row of revalidationRows) hashRowsByKey.set(row.objectKey, row);
  }

  const remainingMediaObjects = mediaObjects.filter((object) => !hashRowsByKey.has(object.key));
  const bytesScheduledForRead = remainingMediaObjects.reduce((sum, object) => sum + (Number(object.size) || 0), 0) +
    revalidationObjects.reduce((sum, object) => sum + (Number(object.size) || 0), 0);
  if (bytesScheduledForRead > maxHashBytes) {
    await writeJson(path.join(auditRoot, 'inventory', 'r2-objects-summary.json'), r2Summary(objects, [...hashRowsByKey.values()]));
    await addBlocker('r2ByteHashes', {
      reason: 'Scheduled R2 media byte reads exceed configured safety limit',
      maxHashBytes,
      bytesScheduledForRead,
      mediaObjectsScheduled: remainingMediaObjects.length,
      fullRehashTriggered,
      neededUserDecision: 'Raise AUDIT_MAX_R2_HASH_BYTES only after confirming no competing sync or backup.'
    });
    process.exitCode = 1;
    return;
  }

  await writeJson(path.join(auditRoot, 'logs', 'r2-prior-hash-reuse.json'), {
    generatedAt: nowIso(),
    priorAuditRoot,
    priorObjectRows: priorObjects.length,
    priorHashRows: priorHashRows.length,
    currentMediaObjects: mediaObjects.length,
    reusableUnchangedObjects: priorReuseCandidates.length,
    revalidatedObjects: revalidationObjects.length,
    revalidationMismatches: hashMismatches.length,
    fullRehashTriggered,
    objectsScheduledForRead: remainingMediaObjects.length,
    bytesScheduledForRead
  });
  await writeJsonl(hashPath, [...hashRowsByKey.values()].sort((a, b) => a.objectKey.localeCompare(b.objectKey)));
  await writeJson(path.join(auditRoot, 'logs', 'r2-hash-resume-state.json'), {
    generatedAt: nowIso(),
    mediaObjects: mediaObjects.length,
    validCurrentRunRows: [...hashRowsByKey.values()].filter((row) => row.evidenceSource === 'current-run-resume').length,
    reusedPriorRows: [...hashRowsByKey.values()].filter((row) => row.evidenceSource === 'verified-2026-06-28-reuse').length,
    revalidatedPriorRows: [...hashRowsByKey.values()].filter((row) => row.evidenceSource === 'prior-hash-revalidation').length,
    remainingHashRows: remainingMediaObjects.length,
    hashConcurrency,
    hashBatchSize
  });
  for (let index = 0; index < remainingMediaObjects.length; index += hashBatchSize) {
    ...
    const batchRows = await mapLimit(batch, hashConcurrency, async (object) =>
      hashR2MediaObject(client, GetObjectCommand, bucket, object, fullRehashTriggered ? 'full-rehash' : 'new-or-changed')
    );
  ...
  const hashRows = [...hashRowsByKey.values()].sort((a, b) => a.objectKey.localeCompare(b.objectKey));
  await writeJsonl(hashPath, hashRows);
...
function r2ObjectFingerprint(object) {
  return hashString([
    object.key,
...
function sameR2ObjectVersion(current, prior) {
  return Number(current.size) === Number(prior.size) &&
    String(current.etag || '').replaceAll('"', '') === String(prior.etag || '').replaceAll('"', '') &&
    String(current.uploadedAt || '') === String(prior.uploadedAt || '');
}

function selectHashRevalidationSample(objects, limit) {
  if (!objects.length || limit <= 0) return [];
  const selected = [];
  const selectedKeys = new Set();
  const byClass = groupedValues(objects, (object) => object.pathClass);
  for (const group of byClass.values()) {
    const classRows = [...group].sort((a, b) => hashString(a.key).localeCompare(hashString(b.key)));
    for (const object of classRows.slice(0, Math.min(8, classRows.length))) {
      selected.push(object);
      selectedKeys.add(object.key);
    }
  }
  const remaining = objects
    .filter((object) => !selectedKeys.has(object.key))
    .sort((a, b) => hashString(a.key).localeCompare(hashString(b.key)));
...
async function hashR2MediaObject(client, GetObjectCommand, bucket, object, evidenceSource = 'current-read') {
  const startedAt = nowIso();
  try {
...
function r2Summary(objects, hashRows) {
  ...
  return {
    ...
    hashCoverage: {
      ...
      byEvidenceSource: countBy(hashRows, (row) => row.evidenceSource || 'unknown')
    }
  };
}
...
async function d1Execute(name, sql) {
  ...
  const wranglerRunner = process.env.AUDIT_WRANGLER_BIN
    ? [process.env.AUDIT_WRANGLER_BIN]
  ...
  const result = await runCommand(`d1-${name}`, 'mise', [
    ...
    '--command',
    sql
  ], { cwd: path.join(repoRoot, 'cloud'), persist: false, maxBuffer: 100 * 1024 * 1024 });
  let parsed = null;
  let parseError = null;
  try {
...
async function d1Inventory() {
  ...
  try {
    ...
    const exports = [
      ...
      ['canonical_snapshot_index', 'd1-canonical-snapshot-index.jsonl'],
      ['canonical_asset_projection', 'd1-canonical-asset-projection.jsonl'],
      ['canonical_storage_object_projection', 'd1-canonical-storage-object-projection.jsonl'],
      ['canonical_prompt_ref_projection', 'd1-canonical-prompt-ref-projection.jsonl'],
      ['canonical_gap_projection', 'd1-canonical-gap-projection.jsonl']
    ];
...
async function metadataInventory() {
  ...
  const metadataObjects = objects.filter((object) =>
    ...
    object.pathClass === 'asset-metadata-v2' ||
    object.pathClass === 'prompt-sidecar' ||
    /backfill-manifest\.[a-f0-9]+\.json$/i.test(object.key) ||
    /\/metadata\/.*\.json$/i.test(object.key)
  );
  const metadataConcurrency = Math.max(1, Number(process.env.AUDIT_R2_METADATA_CONCURRENCY || 8));
  const results = await mapLimit(metadataObjects, metadataConcurrency, async (object) => {
    const startedAt = nowIso();
    try {
      const response = await client.send(new GetObjectCommand({ Bucket: bucket, Key: object.key }));
      const chunks = [];
      ...
      return {
        ...
        reference: { objectKey: object.key, parseStatus: 'read_failed', error: error.message }
      };
    }
  });
  const objectRows = results.map((result) => result.objectRow).sort((a, b) => a.objectKey.localeCompare(b.objectKey));
  const references = results.map((result) => result.reference).sort((a, b) => a.objectKey.localeCompare(b.objectKey));
...
async function reconcile() {
  ...
  for (const row of workerAssets) {
    for (const key of [row.canonicalObjectKey, ...(Array.isArray(row.legacyObjectKeys) ? row.legacyObjectKeys : [])]) {
      if (key) workerByKey.set(key, row);
    }
  }
  ...
  await writeJson(path.join(auditRoot, 'reconciliations', 'worker-raw-delta.json'), {
    ...
    workerAssetsMissingFromR2: workerAssets
      .filter((row) => {
        const keys = [row.canonicalObjectKey, ...(Array.isArray(row.legacyObjectKeys) ? row.legacyObjectKeys : [])].filter(Boolean);
        return keys.length > 0 && !keys.some((key) => r2ByKey.has(key));
      })
      .map((row) => row.canonicalObjectKey)
  });
...
async function validateArtifacts() {
  ...
  const requiredJsonl = [
    ...
    'inventory/d1-metadata-snapshot-index.jsonl'
  ];
  const missing = [];
  const parseFailures = [];
